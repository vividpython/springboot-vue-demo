    ...
    <el-dialog v-model="dialogVisible" title="Tips" width="30%">
      <el-form :model="form" label-width="120px">
        ...
        <el-form-item label="权限标识">
          <el-input v-model="form.perms" style="width: 80%"/>
        </el-form-item>
        <el-form-item label="父ID">
          <el-input v-model="form.parentId" style="width: 80%"/>
        </el-form-item>
  ...
  methods: {
    ...
    async load() {
      try {
        ...
        request.post(`/docperms/${this.currentPage}/${this.pageSize}`, JSON.parse(JSON.stringify(this.formInline))
        ).then(res => {
          console.log(res);
    ...
    save() {
      //更新
      if (this.form.id) {
        request.put("/docperms", this.form).then(res => {
          console.log(res);
          if (res.code === '0') {
            ElMessage({
              message: '修改成功',
        ...
        //新增
        request.post("/docperms", this.form).then(res => {
          console.log(res);
          if (res.code === '0') {
            ElMessage({
              message: '新增成功',
    ...
    handleDelete(id) {
      console.log(id)
      request.delete("/docperms/" + id).then(res => {
        console.log(res);
        if (res.code === '0') {
          ElMessage({
            message: '删除成功',
